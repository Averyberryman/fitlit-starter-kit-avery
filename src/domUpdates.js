...
const displayUserStepGoal = () => {
  userStepsGoal.innerText = `YOU: ${
    currentUser.dailyStepGoal
  } steps vs. THE WORLD: ${averageSteps(mainData.users)} steps`;
};

const displayUserWeeklyStepGoal = () => {
  const weeklyStepsData = getWeeklyStepGoals(currentUser, mainData.activity);
  const weeklyStepGoals = weeklyStepsData
    .map((goalStatus) => {
...
const displayUserDailyHydration = () => {
  dailyHydro.innerHTML = `<div> You've consumed ${getDailyOunces(
    currentUser.id,
    mainData.hydration[mainData.hydration.length - 1].date,
    mainData.hydration
  )}oz of water today! </div>`;
};
...
const displayUserWeeklyHydration = () => {
  ...
  const weeklyHydrationContent = Object.entries(weeklyOuncesData)
    .map(([date, ounces]) => {
...
const displayUserDailySleep = () => {
  dailySleep.innerText = `You slept ${getDailySleepHours(
    currentUser.id,
    mainData.sleep[mainData.sleep.length - 1].date,
    mainData.sleep
  )} hours last night. Your sleep quality was ${getDailySleepQuality(
    currentUser.id,
    mainData.sleep[mainData.sleep.length - 1].date,
    mainData.sleep
  )}.`;
...
const displayUserWeeklySleep = () => {
  const weeklySleepData = getWeeklySleepHoursAndQuality(
    currentUser.id,
    mainData.sleep[mainData.sleep.length - 1].date,
    mainData.sleep
  );
  const weeklySleepContent = Object.entries(weeklySleepData)
    .map(([date, hours]) => {
...
const displayUserAverageSleepData = () => {
  const averageSleepHours = averageDailySleepHours(
    currentUser.id,
    mainData.sleep
  );
  const averageSleepQuality = averageDailySleepQuality(
    currentUser.id,
    mainData.sleep
  );
...
const displayUserSteps = () => {
  ...
  const userSteps = mainData.activity.find(
    (activity) => activity.userID === currentUser.id && activity.date === today
  ).numSteps;
  userStepsToday.innerText = `You have taken ${userSteps} steps today!`;
};

const displayUserMilesWalked = () => {
  milesWalked.innerText = `Miles walked today: ${milesWalkedOnDate(
    currentUser.id,
    mainData.activity[mainData.activity.length - 1].date,
    mainData.users,
    mainData.activity
  )}`;
};
...
const displayUserMinutesActive = () => {
  userMinutesActive.innerText = `Active minutes: ${dailyActiveMinutes(
    currentUser.id,
    mainData.activity[mainData.activity.length - 1].date,
    mainData.activity
  )}`;
