const averageDailySleepHours = (userID, dataSet) => {
  const userSleepData = dataSet.filter(
    (sleepObj) => sleepObj.userID === userID
  );
  const totalSleepHours = userSleepData.reduce(
    (acc, curr) => acc + curr.hoursSlept,
    0
  );
...
const averageDailySleepQuality = (userID, dataSet) => {
  ...
  const totalSleepQuality = userSleepData.reduce(
    (acc, curr) => acc + curr.sleepQuality,
    0
  );
...
const getDailySleepHours = (userID, date, dataSet) => {
  const userSleepData = dataSet.find(
    (sleepObj) => sleepObj.userID === userID && sleepObj.date === date
  );
